...
I seem to recollect that this script can take a couple of minutes to run.
    ...
    <xsl:template match="ncbi:MSPepHit">
        <xsl:value-of select="ancestor::*/ncbi:MSHitSet_number"/>,<xsl:value-of select="ancestor::*/ncbi:MSHits_evalue"/>,<xsl:value-of select="ancestor::*/ncbi:MSHits_pepstring"/>,<xsl:value-of select="ancestor::*/ncbi:MSHits_mass div 1000"/>,<xsl:value-of select="ncbi:MSPepHit_gi"/>,<xsl:value-of 
